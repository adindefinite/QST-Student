...
public class PersonalDao {
	// 个人中心通过学号获取所有用户信息
	public ArrayList<User> getListByName(String us) {
		...
		try {
			...
			while (rs.next()) {
				...
				user.setGender(rs.getInt("gender"));// 性别
				userList.add(user);
			}
	...
	// 个人中心通过学号获取所有成绩信息
	// 获取数据库中所有成绩信息
		public ArrayList<Score> getScoreListByName(String name) {
			PreparedStatement psmt = null;
			ResultSet rs = null;
			ArrayList<Score> scoreList = new ArrayList<Score>();
			// 获取数据库连接
			Connection conn = JDBCUtil.getConnection();
			String sql = "SELECT *,(SELECT count(b.Score) FROM score AS b WHERE a.Score<b.Score and a.Course=b.Course )+1 AS rank FROM score AS a ,`user` AS c WHERE a.`Name`=c.username and c.studentNo=? ORDER BY Course desc,rank;";
			try {
				// 编译sql
				psmt = conn.prepareStatement(sql);
				// 设置参数值
